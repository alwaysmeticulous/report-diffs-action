...
var require_http_retry_utils = __commonJS({
  "node_modules/@alwaysmeticulous/common/dist/http-retry.utils.js"(exports2) {
    ...
    var executeWithRetry = async (operation, options = {}) => {
      const { maxRetries = 3, retryDelay = 1e3, shouldRetry = exports2.defaultShouldRetry, logger } = options;
      let lastError;
...
var require_dist = __commonJS({
  "node_modules/@alwaysmeticulous/common/dist/index.js"(exports2) {
    ...
    var version_utils_1 = require_version_utils();
...
var require_open_socket = __commonJS({
  "node_modules/@alwaysmeticulous/tunnels-client/dist/utils/open-socket.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.openSocket = void 0;
    var net_1 = __importDefault2(require("net"));
    var tls_1 = __importDefault2(require("tls"));
    var createProxyConnection = async (proxyUrl, targetHost, targetPort, useTls) => {
    ...
    var openSocket = async ({ useTls, remoteHost, multiplexingRemotePort, tunnelPassphrase, sendAuthOkAck, logger, onError, onClose }) => {
      let socket;
      const proxyUrl = process.env.HTTPS_PROXY;
      if (proxyUrl) {
        socket = await createProxyConnection(proxyUrl, remoteHost, multiplexingRemotePort, useTls);
      } else if (useTls) {
      ...
      socket.on("error", (err) => {
        ...
        if (err.code === "ECONNREFUSED") {
          const connectionError = new Error(`connection refused: ${remoteHost}:${multiplexingRemotePort} (check your firewall settings)`);
          if (onError) {
    ...
    exports2.openSocket = openSocket;
  }
});
...
var require_get_local_address = __commonJS({
  "node_modules/@alwaysmeticulous/tunnels-client/dist/utils/get-local-address.js"(exports2) {
    ...
    exports2.getLocalAddress = getLocalAddress;
  }
});
...
var require_tunnel_http2_cluster = __commonJS({
  "node_modules/@alwaysmeticulous/tunnels-client/dist/lib/tunnel-http2-cluster.js"(exports2) {
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.TunnelHTTP2Cluster = void 0;
    var events_1 = __importDefault2(require("events"));
    var http_1 = require("http");
    var https_1 = require("https");
    var node_http2_1 = require("node:http2");
    var stream_1 = require("stream");
    var agentkeepalive_1 = __importStar(require_agentkeepalive());
    var cacheable_lookup_1 = __importDefault2(require_source());
    var get_local_address_1 = require_get_local_address();
    var HTTP2_WINDOW_SIZE = 1024 * 1024 * 32;
    var HTTP2_MAX_SESSION_MEMORY = 256;
    var METICULOUS_SIMULATION_HOST = "meticulous-simulation.localhost";
    var TunnelHTTP2Cluster = class extends events_1.default {
      constructor(opts) {
        super();
        this.logger = opts.logger;
        this.opts = opts;
        if (opts.rewriteHostnameToAppUrl && opts.proxyAllUrls) {
          opts.logger.warn("Both --proxyAllUrls and --rewriteHostnameToAppUrl are set. This configuration doesn't make sense, we will only use --rewriteHostnameToAppUrl.");
        }
        this.sockets = opts.sockets;
        this.server = (0, node_http2_1.createServer)({
          settings: {
        ...
        this.dnsCache = new cacheable_lookup_1.default({});
      }
      startListening() {
        const httpAgent = new agentkeepalive_1.default();
        const httpsAgent = new agentkeepalive_1.HttpsAgent(this.opts.allowInvalidCert ? {
        ...
        this.server.on("request", (req, res) => {
          ...
          this.emit("request", {
            ...
            path: (_b = req.url) !== null && _b !== void 0 ? _b : "unknown"
          });
          const {
          ...
          const headersToForward = Object.keys(_headersToForward).reduce((acc, key) => {
            if (!key.startsWith(":")) {
              acc[key] = _headersToForward[key];
            }
            return acc;
          }, {});
          const { hostToRequest, portToRequest, protocolToRequest } = this.getRequestTarget(originalUrl);
          const request = protocolToRequest === "https" ? https_1.request : http_1.request;
          ...
          const clientReq = request({
            ...
            ...(0, get_local_address_1.getLocalAddress)()
          }, (clientRes) => {
            const { connection: connection2, "keep-alive": _, "transfer-encoding": __, ...headersToForward2 } = clientRes.headers;
            res.writeHead(clientRes.statusCode, headersToForward2);
            (0, stream_1.pipeline)(clientRes, res, (err) => {
              if (err) {
                this.logger.error("Response pipeline error", err);
              }
            });
          });
          (0, stream_1.pipeline)(req, clientReq, (err) => {
            if (err) {
      ...
      getRequestTarget(originalUrl) {
        ...
        try {
          ...
          if (!this.opts.proxyAllUrls) {
            this.logger.warn(`Refusing to proxy request to ${hostToRequest} because proxyAllUrls is not set!`);
            return defaultTarget;
          }
          const protocolToRequest = (parsed.protocol || "http").replace(":", "");
          const portToRequest = parsed.port.length > 0 ? parseInt(parsed.port, 10) : protocolToRequest === "https" ? 443 : 80;
          return {
            hostToRequest,
            portToRequest,
            protocolToRequest
    ...
    exports2.TunnelHTTP2Cluster = TunnelHTTP2Cluster;
  }
});
...
var startClusterWorker = async () => {
  ...
  try {
    const socket = await (0, open_socket_1.openSocket)({
      ...
      onError: (err) => {
        ...
        (_a = process.send) === null || _a === void 0 ? void 0 : _a.call(process, { type: "error", data: err.message });
        process.exit(1);
      },
      onClose: () => {
        var _a;
    ...
    const tunnelCluster = new tunnel_http2_cluster_1.TunnelHTTP2Cluster({
      logger,
    ...
    logger.info(`Cluster worker ${options.workerId} is now listening for requests`);
  } catch (error) {
...
humanize-ms/index.js:
   ...
   *)
*/
//# sourceMappingURL=tunnel-worker.entrypoint.js.map
