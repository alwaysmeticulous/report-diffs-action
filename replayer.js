...
const replayEvents = async (options) => {
    ...
    const { appUrl, browser: browser_, outputDir, session, sessionData, replayExecutionOptions, dependencies, screenshottingOptions, generatedBy, } = options;
    // Extract replay metadata
    const { headless, devTools, shiftTime, networkStubbing, skipPauses, padTime, maxDurationMs, maxEventCount, disableRemoteFonts, bypassCSP, moveBeforeClick, } = replayExecutionOptions;
    const metadata = {
        session,
        options: {
            appUrl,
    ...
    (await browser.defaultBrowserContext().pages()).forEach((page) => page.close().catch((error) => {
        logger.error(error);
    }));
    ...
    const originalSessionStartUrl = (0, replay_utils_1.getOriginalSessionStartUrl)({
        session,
        sessionData,
    });
    const startUrl = (0, replay_utils_1.getStartUrl)({ originalSessionStartUrl, appUrl });
    const replayData = await (0, replay_utils_1.initializeReplayData)({ page, startUrl });
    // Set-up network stubbing if required
    if (networkStubbing) {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const networkStubbingExports = require(dependencies.nodeNetworkStubbing
            .location);
        const setupReplayNetworkStubbing = networkStubbingExports.setupReplayNetworkStubbing;
    ...
    const browserContextSeedingExports = require(dependencies.nodeBrowserContext
        .location);
    const setupBrowserContextSeeding = browserContextSeedingExports.setupBrowserContextSeeding;
    ...
    const replayUserInteractions = replayUserInteractionsExports.replayUserInteractions;
    ...
    if (status !== 200) {
        throw new Error(`Expected a 200 status when going to the initial URL of the site (${startUrl}). Got a ${status} instead.`);
    }
    logger.debug(`Navigated to ${startUrl}`);
    // Start simulating user interaction events
    logger.info("Starting simulation...");
    const startTime = luxon_1.DateTime.utc();
    const screenshotsDir = await (0, output_utils_1.prepareScreenshotsDir)(outputDir);
    const storyboard = screenshottingOptions.enabled &&
    ...
    try {
        const replayResult = await replayUserInteractions({
            ...
            ...(sessionDuration != null
                ? { sessionDurationMs: sessionDuration === null || sessionDuration === void 0 ? void 0 : sessionDuration.milliseconds }
                : {}),
...
const shouldHoldBrowserOpen = () => {
    var _a;
    return (((_a = process.env["METICULOUS_HOLD_BROWSER_OPEN"]) !== null && _a !== void 0 ? _a : "").toLowerCase() === "true");
};
const serializeError = (error) => {
    ...
    const message = (_a = toStringOrNull(error.message)) !== null && _a !== void 0 ? _a : toStringOrNull(error);
